...
ECHO.
IF EXIST "%root%karma-simple-mod\" (GOTO :removedirec) ELSE (GOTO :copydirec)
:removedirec
    RMDIR "%root%karma-simple-mod\" /S /Q >nul 2>&1
...
:end
    ...
    ECHO All files should be in the root directory in a folder called karma-simple-mod
	CD /D "%root%"
